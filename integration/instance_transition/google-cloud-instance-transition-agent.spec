...
Summary: "Google Cloud Instance Transition Agent"
Group: Application
License: ASL 2.0
Vendor: Google, Inc.
Provides: google-cloud-instance-transition-agent

%description
"Google Cloud Instance Transition Agent"
...
%files
%defattr(-,root,root)
%attr(755,root,root) %{_bindir}/google_cloud_instance_transition_agent
...
timeout 30 %{_bindir}/google_cloud_instance_transition_agent logusage -s INSTALLED &> /dev/null || true
...
if [ "$1" = "0" ]; then
  ...
  timeout 30 %{_bindir}/google_cloud_instance_transition_agent logusage -s UNINSTALLED &> /dev/null || true
fi
...
if [ "$1" = "0" ]; then
  ...
  timeout 30 %{_bindir}/google_cloud_instance_transition_agent logusage -s UPDATED --agent-version "%{name}-%{VERSION}-%{RELEASE}" &> /dev/null || true
fi
