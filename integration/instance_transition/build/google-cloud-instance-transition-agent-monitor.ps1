...
$LOG_FILE ="$LOGS_DIR\google-cloud-instance-transition-agent-monitor.log"
...
try {
  ...
  if ($status -ne 'Running') {
    Log-Write "Google Cloud Instance Transition Agent service is not running, status: $status"
    Restart-Service -Force $SVC_NAME
    Log-Write 'Google Cloud Instance Transition Agent service restarted'
  }
}
catch {
